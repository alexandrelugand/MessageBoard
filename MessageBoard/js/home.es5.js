...
(function () {
    ...
    messageBoardModule.factory("dataService", ["$http", "$q", function ($http, $q) {
        ...
        var _saveReply = function _saveReply(topic, reply) {

            var deferred = $q.defer();

            $http.post("/api/v1/topics/" + topic.id + "/replies", reply).then(function (result) {
                // Success
    ...
    messageBoardModule.controller("topicsController", ["dataService", function (dataService) {
        ...
        if (!dataService.isReady()) {
            ...
            dataService.getTopics().then(function (result) {
                // Success
            }, function () {
                // Error
    ...
    messageBoardModule.controller("newTopicController", ["$window", "dataService", function ($window, dataService) {
        ...
        vm.save = function () {
            ...
            dataService.addTopic(vm.topic).then(function (result) {
                // Success
                $window.location = "#/";
            }, function () {
                // Error
    ...
    messageBoardModule.controller("singleTopicController", ["$window", "dataService", "$routeParams", function ($window, dataService, $routeParams) {
        ...
        vm.addReply = function () {
            ...
            dataService.saveReply(vm.topic, vm.reply).then(function (result) {
                // Success
                vm.reply.body = "";
            }, function () {
                // Error
